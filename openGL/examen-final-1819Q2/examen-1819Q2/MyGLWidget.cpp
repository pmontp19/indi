...
{
  radiEsc = sqrt(80);
  radiEsc = glm::distance(glm::vec3(10.0, 4.0, 7.0), glm::vec3(-10.0, 0.0, -7.0))*0.5f;
...
void MyGLWidget::modelTransformParet1 ()
{
  ...
  TG = glm::scale(TG, glm::vec3(20.0, 2.0, 0.2));
  glUniformMatrix4fv (transLoc, 1, GL_FALSE, &TG[0][0]);
}
...
void MyGLWidget::projectTransform ()
{
  ...
  if (cameraPrimeraPersona) {
      Proj = glm::perspective(float(M_PI/2.0), ra, 0.1f, -25.0f);
  } else {
      Proj = glm::perspective(float(M_PI/2.5), ra, radiEsc, 3.0f*radiEsc);
  }
...
void MyGLWidget::viewTransform ()
{
  ...
  glm::vec3 vrp = posNinot;
  glm::vec3 obs = posNinot;
  vrp[0] += -5.0; // vrp a la direccio de -x
  obs[0] += -.1;
  obs[1] += 4.0;
  if (cameraPrimeraPersona) {
      // glm::lookAt (OBS, VRP, UP)
      View = lookAt (obs, vrp, glm::vec3(0,1,0));
  } else {
      View = glm::translate(glm::mat4(1.f), glm::vec3(0, 0, -2*radiEsc));
      View = glm::rotate(View, anglePsi, glm::vec3(1,0,0));
      View = glm::rotate(View, angleTheta, glm::vec3(0,1,0));
      //View = glm::rotate(View, -angleY, glm::vec3(0, 1, 0));
      //View = glm::lookAt (glm::vec3(15.0,5.0,0), glm::vec3(0,0,0), glm::vec3(0,1,0));
  }
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
    ...
    case Qt::Key_C: {
        cameraPrimeraPersona = !cameraPrimeraPersona;
        emit enviaEstatPrimera(cameraPrimeraPersona);
        emit enviaEstatTercera(!cameraPrimeraPersona);
        break;
    }
...
void MyGLWidget::resetPosicio() {
    makeCurrent();
    posNinot = glm::vec3(9.0, 0.0, 0.0);  // posició inicial del porter
    update();
}

void MyGLWidget::canviaPrimeraP() {
    makeCurrent();
    cameraPrimeraPersona = true;
    projectTransform();
    viewTransform();
    update();
}

void MyGLWidget::canviaTerceraP() {
    makeCurrent();
    cameraPrimeraPersona = false;
    projectTransform();
    viewTransform();
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e)
{
  ...
  if (DoingInteractive == ROTATE)
  {
    ...
    angleTheta += (e->x() - xClick) * (float(M_PI) / 180);
    anglePsi += (e->y() - yClick) * (float(M_PI) / 180);
