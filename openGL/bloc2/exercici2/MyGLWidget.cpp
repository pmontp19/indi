...
void MyGLWidget::iniEscena ()   // Cal modificar aquesta funció...
{
  ...
  creaBuffersTerra();
  creaBuffersArbre();
  escalaPat = 2.0 * escalaPat;

  // centreEsc = glm::vec3 (1, 0, 1);
  centreEsc = glm::vec3(0,0,0) + glm::vec3(5,3,5);
  centreEsc = centreEsc / glm::vec3(2.0);
  // radiEsc = 3.5;
  radiEsc = glm::distance(glm::vec3(0,0,0), glm::vec3(5,3,5));
...
{
  float angle = -135 * float(M_PI)/180;
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::translate(TG, glm::vec3(4,0,4));
  TG = glm::rotate(TG, angle, glm::vec3(0,1.0,0));
...
void MyGLWidget::viewTransform ()      // Cal modificar aquesta funció...
{
  ...
  // View = glm::lookAt(centreEsc + glm::vec3(0, 2, 2*radiEsc), centreEsc, glm::vec3(0, 1, 0));
...
void MyGLWidget::keyPressEvent(QKeyEvent* event)  // Cal modificar aquesta funció...
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_Up: { 
      ...
      if (angleX < (20 * float(M_PI)/180) ) {
        angleX += 20 * float(M_PI)/180;
      }
      break;
    }
    case Qt::Key_Down: { 
