...
void MyGLWidget::iniEscena ()
{
  ...
  // ja en coordenades SCO
  posFocus = glm::vec3(0,0,0);
  glUniform3f(colLoc, 0.8, 0.8, 0.8);
  glUniform3f(llumLoc, 0.2, 0.2, 0.2);
...
void MyGLWidget::viewTransform ()
{
  ...
  reassignaPosFocus(); // necessari per recalcular al canviar de posicio
  if (f == patricio || f == arbre) {
      glm::vec4 posFocusSCO = View * glm::vec4(posFocus,1.0);
      posFocus = glm::vec3(posFocusSCO.x, posFocusSCO.y, posFocusSCO.z);
  }
  glUniform3fv(posLoc,1,&posFocus[0]);
  glUniformMatrix4fv (viewLoc, 1, GL_FALSE, &View[0][0]);
}

void MyGLWidget::reassignaPosFocus()
{
  switch (f)
  {
  case camera:
    ...
    break;
  case patricio:
    posFocus = glm::vec3(4, 3.5, 4);
    break;
  case arbre:
    posFocus = glm::vec3(1, 3.5, 1);
    break;
  }
}

void MyGLWidget::tractaOpcio(QString label) {
    makeCurrent();
    if (label.toLower() == "camera") {
        f = camera;
        posFocus = glm::vec3(0,0,0);
    } else if (label.toLower() == "patricio") {
        f = patricio;
        posFocus = glm::vec3(4,3.5,4);
    } else if (label.toLower() == "arbre") {
        f = arbre;
        posFocus = glm::vec3(1,3.5,1);
    }
    viewTransform();
