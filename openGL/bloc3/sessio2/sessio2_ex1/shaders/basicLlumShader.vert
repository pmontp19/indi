...
void main()
{
  ...
  vec3 nomNormalMatrix = normalize (normalMatrix * normal);
  
  // passar posicio vertex a SCO esta en SCM
  //vec3 vertexSCO = view * TG * vertex;  
  vec4 vertexSCO = view * TG * vec4 (vertex, 1.0);  
  
  // passar posFocus a SCO focus esta en SCA
  // vec3 posFocusSCO = view * posFocus; 
  //vec4 posFocusSCO = view * vec4 (posFocus, 1.0); 
  vec4 posFocusSCO = vec4(posFocus, 1.0);
  
  vec4 vecL = posFocusSCO - vertexSCO;
  vec3 vecLxyz = normalize(vecL.xyz);


  // fcolor = Lambert(nomNormalMatrix, vecLxyz);
  fcolor = Phong(nomNormalMatrix, vecLxyz, vertexSCO);
