...
{
  // radiEsc = sqrt(5);  
  radiEsc = distance (glm::vec3(-2,-1,-2), glm::vec3(2,3,2)) / 2.0f;

  posFocus = glm::vec3(-2,0,0);
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  TG = glm::translate(TG, posPatr);
  TG = glm::rotate(TG, float(M_PI/2), glm::vec3(0, 1, 0)); // gir 45 graus
...
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::translate(TG, glm::vec3(2,0.25,0));
...
void MyGLWidget::projectTransform ()
{
  ...
  if (perspectiva && fpv)
    Proj = glm::perspective(float(M_PI/2.0), ra, zn, zf);
  else if (perspectiva && !fpv)
...
void MyGLWidget::viewTransform ()
{
  ...
  glm::mat4 View;
  if (fpv) {
    View = glm::lookAt (posPatr, glm::vec3(2,0.25,0), up);
  } else {
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e)
{
  ...
  {
    // TODO el limit no funciona
    if (fov > 0.1f && fov < float(M_PI) ) {
      fov += (e->y() - yClick) * zoomFactor;
      projectTransform();
    }
  }
...
glm::vec3 MyGLWidget::calculaCapsaModel (const Model &model)
{
  // Càlcul capsa contenidora i valors transformacions inicials
  float minx, miny, minz, maxx, maxy, maxz;
...
float MyGLWidget::escalarModel(const Model &model, float factor) {
...
void MyGLWidget::createBuffersPatricio ()
{
  ...
  centrePatr = calculaCapsaModel(patr);
...
void MyGLWidget::createBuffersVaca ()
{
  ...
  // Calculem la capsa contenidora del model
  centreVaca = calculaCapsaModel (vaca);
  escalaVaca = escalarModel (vaca, 0.5);
...
void MyGLWidget::carregaShaders()
{
  ...
  posLoc = glGetUniformLocation (program->programId(), "posLoc");
}
